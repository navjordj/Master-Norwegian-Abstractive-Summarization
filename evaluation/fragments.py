...
class Fragments(object):
    ...
    @classmethod
    def _load_model(cls):
        # Replacing _en with _nb for Norwegian
        if not hasattr(cls, "_en"):

            try:

                cls._en = _spacy.load("nb_core_news_sm")

            except:

                _system("python -m spacy download nb_core_news_sm")
                cls._en = _spacy.load("nb_core_news_sm")
...
if __name__ == '__main__':
    ...
    summaries = test["ingress"].values
    texts = test["article"].values
