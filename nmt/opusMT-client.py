...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--target-language", type=str, default='no')
    ...
    # open connection
    if args.ssl == 1:
        ws = create_connection("wss://{}:{}/translate".format(args.host,
                               args.port), sslopt={"cert_reqs": ssl.CERT_NONE})

    else:
        ws = create_connection(
            "ws://{}:{}/translate".format(args.host, args.port))
    ...
    for line in sys.stdin:
        ...
        if count == args.batch_size:
            # translate the batch
            data = {'text': message, 'source': args.source_language,
                    'target': args.target_language, 'model': args.model}
    ...
    if count:
        ...
        # print("send batch " + batch)
        data = {'text': batch, 'source': args.source_language,
