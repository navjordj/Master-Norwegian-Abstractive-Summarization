...
class Translate(WebSocket):
    ...
    def handleMessage(self):
        ...
        if len(srctxt) > args.max_input_length:
            self.sendMessage(json.dumps({'result': 'ERROR: Input too long! Maximum length = {}'.format(
                args.max_input_length)}, sort_keys=True, indent=4))
            return

        if not fromLang:
            isReliable, textBytesFound, details = cld2.detect(
                srctxt, bestEffort=True)
        ...
        if not model in opusMT:
            ...
            self.sendMessage(json.dumps(
                {'result': 'ERROR: unsupported language pair ' + langpair}, sort_keys=True, indent=4))
            return
        ...
        try:
            ...
            print("invalid json: %s" % error)
            self.sendMessage(json.dumps(
                {'result': 'ERROR: {}'.format(error)}, sort_keys=True, indent=4))
